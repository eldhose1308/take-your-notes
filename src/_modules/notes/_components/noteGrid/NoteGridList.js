...
import { getNotesOfSelectedFile, getSelectedFile, getSelectedFolder, getSelectedNote } from "store/selectors/notesSelectors";

const NoteGridList = (props) => {
    // const { notes: notesList = [] } = props;

    const notesObj = useSelector(getNotesOfSelectedFile) || [];
    ...
    return (
        <div className='overflow-scroll my-2 pr-4 h-screen-3/4'>
            {Object.values(notesObj).map((item, index) => {
                const { id } = item
